...
/**
 * <h3>General usage description</h3>
 * Server creates its own core SrvApi based on passed baseApi. The following
 * interfaces are used:
 * <p>
 * 
 * <ul>
 ...
 * <tt>Registration mode</tt> and invokes do Registration() for each plugin
 * from the actual plugin list.
 * 
 ...
 * doDataPreparation() enters <tt>Running mode</tt>, logs in as
 * <tt>USystem</tt> user and invokes all registered data preparation
 * procedures.
 * <p>
 * 
...
public class BasicServer implements IBasicServer, ISORegistration, ISOQuery, ISODbInfo,
        ITime{
    ...
    public BasicServer(Api baseApi){
        ...
        if (null == m_coreApi.queryIntfImplementor(ITime.class)) {
            m_coreApi.setIntfImplementor(ITime.class, this);
        }
        // install IProfiler interface
        if (null == m_coreApi.queryIntfImplementor(IProfiler.class)) {
            m_coreApi.setIntfImplementor(IProfiler.class, new Profiler());
        }
        // install ISOQuery interface
        m_coreApi.setIntfImplementor(ISOQuery.class, this);
        
        m_coreApi.setIntfImplementor(IDatabaseInfo.class, getDbInfo((IPooledConnection) m_coreApi.getIntfImplementor(IPooledConnection.class)));
    ...
    private void loadRegLists(Connection connection) throws SQLException {
        ...
        m_upRegList = ((EntityJournal<UpgradeProcedure>) getEntity(UP_TABLE))
                .exclude(connection, getTableDbName(UP_TABLE),
                        upList);

        m_dppRegList = ((EntityJournal<DataPreparationProcedure>) getEntity(DPP_TABLE))
                .exclude(connection,
                        getTableDbName(DPP_TABLE), dppList);
     ...
     * (non-Javadoc)
     * 
     * @see com.triniforce.server.srvapi.ISORegistration#registerSelectFunction(com.triniforce.server.srvapi.SelectFunctionDef)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.triniforce.server.srvapi.ISORegistration#registerTableDef(com.triniforce.db.ddl.TableDef)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.triniforce.server.srvapi.ISORegistration#registerUpgradeProcedure(com.triniforce.server.srvapi.UpgradeProcedure)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.triniforce.server.srvapi.ISORegistration#registerDataPreparationProcedure(com.triniforce.server.srvapi.DataPreparationProcedure)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.triniforce.server.srvapi.ISODbInfo#getTableDbName(java.lang.String)
     */
    	...
    	runInSingleConnectionMode(new ICallback(){
			public void call() throws Exception{
		        ...
		        for (DataPreparationProcedure proc : m_dppRegList) {
		            PSI psi = ApiAlgs.getProfItem("Server initialization", proc.getEntityName());
		            try{
		                ApiAlgs.getLog(this).trace(
		                    String.format("Data preparation procedure: \"%s\"", proc.getEntityName())); //$NON-NLS-1$
		                proc.run();
		            }finally{
    ...
    private void flushEntities(Connection connection) throws Exception {
        ...
        if (!m_upRegList.isEmpty()) {
            ...
            ((EntityJournal<UpgradeProcedure>) getEntity(UP_TABLE))
                    .add(connection, getTableDbName(UP_TABLE),
                            m_upRegList);
        }
    }
    ...
    public void enterMode(final Mode mode) {
        ...
        /*
        if (mode.equals(Mode.Running)) {
            if (m_runningApi == null)
                throw new EInvalidServerState(mode);
        }
        */
        /*
...

        {
            ...
            switch (mode) {
            case Registration:
                setRegistrationInterfaces(newApi);
                break;
            case Upgrade:
                setUpgradeInterfaces(newApi);
                break;
            case Running:
                setRunningInterfaces(newApi);
                break;
            }
        }

        ApiStack newApiStack = new ApiStack();
        newApiStack.getStack().push(m_coreApi); // server core api
        if (mode.equals(Mode.Running)) {
        ...
        try {
        	...
        	runInSingleConnectionMode(new ICallback(){
				public void call() throws Exception {
		            ...
		            Connection connection = ApiStack.getApi().getIntfImplementor(
		                    Connection.class);
		            UpgradeRunner player = new UpgradeRunner((Connection) connection,
		                    m_actualTabStates);
		            List<DBOperation> cl = m_desiredTables.getCommandList();
		            player.run(cl);
				}
	...
	public void finit() {
		//enterMode(Mode.Running);
		//try{
			ListIterator<IPlugin> i = m_plugins.listIterator(m_plugins.size());
			while(i.hasPrevious()){
				IPlugin plugin = i.previous();
				try{
					plugin.finitApi();
				} catch(Throwable t){
					ApiAlgs.getLog(this).error("Plugin finit problem", t);//$NON-NLS-1$
				}
			}
		//} finally{
			//leaveMode();
    ...
    public void startPeriodicalTasks() {
        for(BasicPeriodicalTask task: m_tasks){
            m_ptExecutor.scheduleWithFixedDelay(task, task.initialDelay, task.delay, task.unit);            
        }
    }
